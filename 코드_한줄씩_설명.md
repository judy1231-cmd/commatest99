...
| 분류 | 파일 수 | 이 문서에서 설명한 파일 |
|------|--------:|----------------------|
| Java (백엔드) | 104개 | 5개 (User, ApiResponse, JwtUtil, JwtInterceptor, WebConfig, CorsConfig, AuthMapper, AuthService, AuthController) |
| JSX/JS (프론트엔드) | 83개 페이지 + 12개 유틸 | 6개 (fetchWithAuth, Button, Input, Toast, PrivateRoute) |
| XML MyBatis (SQL) | 17개 | 1개 (AuthMapper.xml) |
| YML 설정 | 14개 | - |
| **합계** | **약 255개** | **15개 (핵심 파일만)** |

> 이 문서는 전체 255개 파일 중 **아키텍처를 이해하는 데 가장 중요한 15개 핵심 파일**을 설명한다.
> 나머지 240개 파일은 이 15개 파일의 패턴을 그대로 반복하거나 특정 도메인에만 적용된다.
...
│   └── src/
│       ├── api/
│       │   └── fetchWithAuth.js ★          ← JWT 자동갱신 fetch wrapper
│       │
│       ├── components/
│       │   ├── common/
│       │   │   ├── Button.jsx ★            ← 공통 버튼 (primary/outline/ghost)
│       │   │   ├── Input.jsx ★             ← 공통 인풋 (아이콘 지원)
│       │   │   ├── Toast.jsx ★             ← 알림 메시지 (3초 자동 닫힘)
│       │   │   └── PrivateRoute.jsx ★      ← 로그인/관리자 라우트 보호
│       │   ├── user/
│       │   │   ├── UserNavbar.jsx          ← 하단 네비게이션 바
│       │   │   └── ActivityModal.jsx       ← 활동 상세 모달
│       │   └── admin/
│       │       ├── AdminHeader.jsx         ← 관리자 상단 헤더
...
│           │   ├── auth/
│           │   │   ├── Login.jsx           ← 로그인
│           │   │   ├── Signup.jsx          ← 회원가입 (3단계 플로우)
│           │   │   ├── SignupComplete.jsx   ← 가입완료
│           │   │   ├── PasswordReset.jsx   ← 비밀번호 재설정
│           │   │   ├── AuthLogin.jsx       ← 인증 전용 로그인
│           │   │   ├── AuthSignup.jsx      ← 인증 전용 가입
│           │   │   ├── AuthFindPassword.jsx← 인증 전용 비번찾기
│           │   │   └── AuthVerifyEmail.jsx ← 이메일 인증 대기
│           │   │
│           │   ├── diagnosis/
│           │   │   ├── DiagnosisHome.jsx   ← 진단 랜딩
│           │   │   ├── DiagnosisStart.jsx  ← 진단 시작 안내
│           │   │   ├── DiagnosisQuiz.jsx   ← 설문 12문항
│           │   │   └── DiagnosisResult.jsx ← 진단 결과 (유형+점수)
│           │   │
│           │   ├── recommend/
│           │   │   ├── RecommendHome.jsx   ← 추천 메인
│           │   │   └── RecommendHistory.jsx← 추천 기록
│           │   │
│           │   ├── mypage/
│           │   │   ├── MypageHome.jsx      ← 마이페이지 홈
│           │   │   ├── MypagePassword.jsx  ← 비번 변경
│           │   │   ├── MypageStatus.jsx    ← 계정 상태
│           │   │   └── MypageWithdraw.jsx  ← 회원 탈퇴
│           │   │
│           │   ├── records/
│           │   │   ├── RecordsRest.jsx     ← 휴식 기록 목록
│           │   │   ├── RecordsDiagnosis.jsx← 진단 기록
│           │   │   └── RecordsEmotion.jsx  ← 감정 기록
│           │   │
│           │   ├── stats/
│           │   │   ├── StatsRest.jsx       ← 휴식 통계
│           │   │   ├── StatsDiagnosis.jsx  ← 진단 통계
│           │   │   └── StatsEmotion.jsx    ← 감정 통계
│           │   │
│           │   ├── contents/
│           │   │   ├── ContentsList.jsx    ← 콘텐츠 목록
│           │   │   └── ContentsDetail.jsx  ← 콘텐츠 상세
│           │   │
│           │   ├── MainDashboard.jsx       ← 메인 대시보드
│           │   ├── HeartRateCheck.jsx      ← 심박 측정 + QR공유
│           │   ├── RestTypeTest.jsx        ← 휴식 유형 테스트
│           │   ├── StressTest.jsx          ← 스트레스 테스트
│           │   ├── MapPage.jsx             ← Leaflet 장소 지도
│           │   ├── PlaceDetail.jsx         ← 장소 상세 + 리뷰
│           │   ├── RestRecord.jsx          ← 휴식 기록 등록
│           │   ├── Rest[7가지유형].jsx     ← 유형별 안내 (7개)
│           │   ├── Settings.jsx            ← 설정 홈
│           │   ├── SettingsProfile.jsx     ← 프로필 설정
│           │   ├── SettingsSecurity.jsx    ← 보안 설정
│           │   ├── SettingsPreferences.jsx ← 개인 설정
│           │   ├── Notifications.jsx       ← 알림 목록
│           │   ├── Community.jsx           ← 커뮤니티 (2차)
│           │   ├── CommunityDetail.jsx     ← 게시글 상세 (2차)
│           │   ├── CommunityWrite.jsx      ← 글쓰기 (2차)
│           │   ├── Challenge.jsx           ← 챌린지 (2차)
│           │   ├── KakaoConfirm.jsx        ← 카카오 로그인 처리
│           │   ├── GoogleConfirm.jsx       ← 구글 로그인 처리
│           │   ├── NaverConfirm.jsx        ← 네이버 로그인 처리
│           │   └── OAuthCallback.jsx       ← 소셜 공통 콜백
│           │
│           └── admin/                      ─ 관리자 페이지 (26개)
│               ├── AdminLogin.jsx          ← 관리자 로그인
│               ├── AdminDashboard.jsx      ← 대시보드
│               ├── UserManagement.jsx      ← 사용자 목록
│               ├── PlaceApproval.jsx       ← 장소 승인
│               ├── CommunityManagement.jsx ← 커뮤니티 관리
│               ├── ChallengeManagement.jsx ← 챌린지 관리
│               ├── Analytics.jsx           ← 통계 분석
...
│       │   │   ├── JwtInterceptor.java ★   ← 요청 인증 처리
│       │   │   ├── WebConfig.java ★        ← 인터셉터 등록
│       │   │   ├── CorsConfig.java ★       ← CORS 허용 설정
│       │   │   └── ClaudeApiClient.java    ← Claude API 연동
│       │   ├── util/
│       │   │   ├── ApiResponse.java ★      ← 공통 응답 포맷
│       │   │   ├── MailService.java        ← 이메일 발송 (Gmail SMTP)
│       │   │   ├── FileUploadService.java  ← 파일 업로드
│       │   │   ├── WeatherService.java     ← 기상청 API
│       │   │   └── YoloService.java        ← YOLO 이미지 분석 연동
│       │   ├── exception/
│       │   │   └── GlobalExceptionHandler.java ← 전역 예외 처리
│       │   └── scheduler/
│       │       └── PlaceScoreScheduler.java    ← 매일 새벽 3시 점수 갱신
│       │
│       └── domain/                         ─ 15개 도메인 (각 4세트)
│           │
│           ├── auth/                       ─ 인증 (9개)
│           │   ├── controller/
│           │   │   ├── AuthController.java ★   ← 회원가입/로그인/토큰갱신
│           │   │   ├── KakaoAuthController.java← 카카오 OAuth2
│           │   │   ├── GoogleAuthController.java← 구글 OAuth2
│           │   │   └── NaverAuthController.java ← 네이버 OAuth2
│           │   ├── service/
...
│           │   └── mapper/
│           │       └── AuthMapper.java ★        ← DB 쿼리 인터페이스
│           │
│           ├── user/                       ─ 사용자 (5개)
│           │   ├── controller/ UserController.java
│           │   ├── service/    UserService.java
│           │   ├── mapper/     UserMapper.java
│           │   └── model/      User.java ★ / UserSettings.java
│           │
│           ├── survey/                     ─ 설문 (6개)
│           │   ├── controller/ SurveyController.java
│           │   ├── service/    SurveyService.java / RestTypeClassifier.java
│           │   ├── mapper/     SurveyMapper.java
│           │   └── model/      SurveyQuestion / SurveyChoice / SurveyResponse
│           │
│           ├── diagnosis/                  ─ 진단 (6개)
│           │   ├── controller/ DiagnosisController.java
│           │   ├── service/    DiagnosisService.java
│           │   ├── mapper/     DiagnosisMapper.java
│           │   └── model/      DiagnosisResult / HeartRateMeasurement / MeasurementSession / RestTypeScore
│           │
│           ├── rest/                       ─ 휴식 기록 (5개)
│           │   ├── controller/ RestLogController.java
│           │   ├── service/    RestLogService.java
│           │   ├── mapper/     RestLogMapper.java
│           │   └── model/      RestLog / RestType / RestActivity
│           │
│           ├── place/                      ─ 장소 (8개)
│           │   ├── controller/ PlaceController / PlaceSeedController
│           │   ├── service/    PlaceService / PlaceSeedService
│           │   ├── mapper/     PlaceMapper.java
│           │   └── model/      Place / PlacePhoto / PlaceTag / PlaceReview / PlaceBookmark
│           │
│           ├── recommendation/             ─ 추천 (4개)
│           ├── stats/                      ─ 통계 (4개)
│           ├── badge/                      ─ 배지 (4개)
│           ├── notification/               ─ 알림 (4개)
│           ├── contents/                   ─ 콘텐츠 (4개)
│           ├── community/                  ─ 커뮤니티 2차 (7개)
│           ├── challenge/                  ─ 챌린지 2차 (4개)
│           ├── admin/                      ─ 관리자 (5개)
│           └── insight/                    ─ 인사이트 (2개)
│
│       MyBatis SQL 쿼리 파일
│       ├── resources/mapper/  (17개 XML)
│       │   ├── auth/AuthMapper.xml ★
│       │   ├── user/UserMapper.xml
│       │   ├── survey/SurveyMapper.xml
│       │   ├── diagnosis/DiagnosisMapper.xml
│       │   ├── rest/RestLogMapper.xml
│       │   ├── place/PlaceMapper.xml
│       │   ├── recommendation/RecommendationMapper.xml
│       │   ├── stats/StatsMapper.xml
│       │   ├── badge/BadgeMapper.xml
│       │   ├── notification/NotificationMapper.xml
│       │   ├── community/ (Post/Comment/PostPhoto) 3개
│       │   ├── challenge/ChallengeMapper.xml
│       │   ├── contents/ContentsMapper.xml
│       │   └── admin/ (Admin/AdminSettings) 2개
│       │
│       └── resources/
...
│       │  HTTP 요청 (이미지 분류)
│       ▼
├── yolo_service/  ───────────────────────────── Python FastAPI (포트 8090)
│   ├── main.py                         ← YOLO 이미지 분석 API
│   └── requirements.txt
│
├── mobile/  ─────────────────────────────────── React Native (미완성)
│
├── [데이터베이스]
│   ├── MySQL (Cloud SQL)               ← 27개 테이블
│   └── Redis (Upstash)                 ← Refresh Token 저장
│
└── [설정 파일]
    ├── docker-compose.yml              ← 로컬 MySQL + Redis
    ├── 쉼표_DDL.sql                    ← DB 테이블 생성 SQL
    ├── 쉼표_ERD.html                   ← ERD 시각화
    ├── 코드_설계_이유.md               ← 왜 이렇게 만들었는지
    └── 코드_한줄씩_설명.md             ← 이 파일
```
...
[Login.jsx]
   ...
   ▼
[JwtInterceptor] ── PUBLIC_PATH → 통과
   ▼
[AuthController] ── 파라미터 검증
   ▼
[AuthService] ──── BCrypt 비밀번호 검증
   │              ── JWT 토큰 발급
   │              ── Redis에 RefreshToken 저장
   ▼
[AuthMapper.xml] ── SELECT * FROM users WHERE username = ?
   ▼
[MySQL]
   │
...
[fetchWithAuth.js]
   ...
   ▼
[JwtInterceptor] ── 토큰 검증 → request.setAttribute("쉼표번호")
   ▼
[각 Controller] ── request.getAttribute("쉼표번호") 로 사용자 확인
   ▼
[각 Service]    ── 비즈니스 로직
   ▼
[각 Mapper.xml] ── SQL 실행
...
[MySQL]
   │
   │ ApiResponse { success, data, message } 반환
   ▼
[각 페이지 컴포넌트] ── data.success 체크 → UI 업데이트
```
...
Controller.java   ──── HTTP 요청 받기, 응답 내보내기
    │ 호출
    ▼
Service.java      ──── 비즈니스 로직, 트랜잭션
    │ 호출
    ▼
Mapper.java       ──── SQL 메서드 인터페이스 (MyBatis가 구현체 자동 생성)
    │ 연결
    ▼
