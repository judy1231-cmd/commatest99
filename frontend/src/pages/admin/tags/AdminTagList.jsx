...
function AdminTagList() {
  ...
  const loadTags = async () => {
    setLoading(true);
    try {
      const data = await fetchWithAuth('/api/admin/tags');
  ...
  const handleDelete = async (id, tagName) => {
    if (!window.confirm(`"${tagName}" 태그를 모든 장소에서 삭제할까요?`)) return;
    try {
      const data = await fetchWithAuth(`/api/admin/tags/${id}`, { method: 'DELETE' });
      if (data.success) {
  ...
  const maxUsage = tags.length > 0 ? Math.max(...tags.map(t => Number(t.usageCount) || 0)) : 1;
  const totalUsage = tags.reduce((s, t) => s + (Number(t.usageCount) || 0), 0);

  const filtered = useMemo(() => {
    let list = tags.filter(t => {
      if (typeFilter && t.restType !== typeFilter) return false;
      if (search && !t.tagName?.includes(search)) return false;
      return true;
    });
    if (sortBy === 'usage') list = [...list].sort((a, b) => (Number(b.usageCount) || 0) - (Number(a.usageCount) || 0));
    if (sortBy === 'name')  list = [...list].sort((a, b) => (a.tagName || '').localeCompare(b.tagName || '', 'ko'));
    return list;
  }, [tags, search, sortBy, typeFilter]);
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              { icon: 'label',      label: '총 태그 수',   value: `${tags.length}개`,         iconCls: 'bg-blue-50 text-blue-600'    },
              { icon: 'category',   label: '유형 분류',    value: `${REST_TYPES.length}가지`,  iconCls: 'bg-violet-50 text-violet-600' },
              { icon: 'place',      label: '총 장소 태그', value: `${totalUsage}건`,           iconCls: 'bg-emerald-50 text-emerald-600' },
              { icon: 'touch_app',  label: '현재 표시',    value: `${filtered.length}개`,      iconCls: 'bg-amber-50 text-amber-600'  },
            ].map((card, i) => (
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-gray-100 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div className="relative flex-1 max-w-xs">
                  ...
                  <input
                    ...
                    placeholder="태그 검색"
                    className="w-full h-9 pl-9 pr-4 border border-gray-200 rounded-lg text-sm text-gray-700 placeholder:text-gray-400 bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
                  />
                </div>

                <div className="text-xs text-gray-400 ml-auto">
                  <span className="font-semibold text-gray-600">{filtered.length}</span>개 표시
                  {(search || typeFilter) && (
                    <button
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {REST_TYPES.map(t => {
                  const count = tags.filter(tag => tag.restType === t.key).length;
                  return (
                    <button
                      key={t.key}
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      ...
                      <th className="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">사용 장소 수</th>
                  ...
                  <tbody>
                    {filtered.length === 0 ? (
                      ...
                      filtered.map((tag, idx) => {
                        ...
                        const usageCount = Number(tag.usageCount) || 0;
                        const barPct = maxUsage > 0 ? Math.round((usageCount / maxUsage) * 100) : 0;
                          ...
                          >
                            ...
                            <td className="px-5 py-3.5">
                              {typeInfo ? (
                                ...
                                <span className="text-xs text-gray-300">{tag.restType || '—'}</span>
                              )}
                            </td>

                            <td className="px-5 py-3.5">
                              <div className="flex items-center gap-3">
                                <span className="text-sm font-bold text-gray-800 tabular-nums w-8 shrink-0">{usageCount}</span>
                                <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden max-w-[120px]">
                                  <div
                                    className="h-full rounded-full transition-all"
                                    style={{ width: `${barPct}%`, backgroundColor: typeInfo?.color || '#10B981' }}
                                  />
                                </div>
                              </div>
                            </td>
            ...
            <div className="px-5 py-3 border-t border-gray-100">
              <p className="text-xs text-gray-400">* 태그는 장소 데이터에 귀속됩니다. 삭제 시 해당 태그명을 가진 모든 장소 태그가 제거됩니다.</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <div className="grid grid-cols-4 gap-3">
              {REST_TYPES.map(type => {
                const count = tags.filter(t => t.restType === type.key).length;
                const usage = tags.filter(t => t.restType === type.key).reduce((s, t) => s + (Number(t.usageCount) || 0), 0);
                return (
                  <div
                    key={type.key}
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-gray-400 mt-0.5">
                        ...
                        사용 <span className="font-semibold text-gray-600">{usage}</span>건
                      </p>
                    </div>
                  </div>
                );
