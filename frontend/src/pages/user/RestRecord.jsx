    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        if (location.state?.fromContents && location.state?.contentCategory) {
          ...
          const now = new Date();
          const duration = location.state.contentDuration || 30;
          ...
          setForm((prev) => ({
            ...prev,
            restTypeId: matched ? String(matched.id) : prev.restTypeId,
            memo: location.state.contentTitle || '',
            startTime: toLocal(now),
            endTime: toLocal(end),
          }));
          setShowModal(true);
        ...
        {(() => {
          const chartData = [...logs]
            ...
            .map(l => ({
              ...
              향상: l.emotionBefore != null && l.emotionAfter != null
                ? l.emotionAfter - l.emotionBefore : null,
            }));
            ...
            <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm mb-6">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={chartData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
                  ...
                  />
                  <Line
                    type="monotone" dataKey="전"
                    stroke="#cbd5e1" strokeWidth={2} strokeDasharray="5 3"
                    dot={{ r: 3, fill: '#cbd5e1', strokeWidth: 0 }}
                    activeDot={{ r: 5 }}
                    connectNulls
                  />
                  <Line
                    type="monotone" dataKey="후"
